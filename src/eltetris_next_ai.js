                    ...
                    for (var nj = 0; nj < this.number_of_columns - next_piece[ni].width + 1; nj++) {
                        ...
                        if (!nlast_move.game_over) {
                            ...
                            if (nlast_move.landing_height < 17) { // 贪婪
                                var sum = nlast_move.rows_removed + last_move.rows_removed;
                                if (sum <= 2) {
                                    weight = 1;
                                } else {
                                    weight = factorial(last_move.rows_removed) + factorial(nlast_move.rows_removed);
                                }
                                // if (last_move.rows_removed == 0 && nlast_move.rows_removed == 0) {
                                //     weight = 1;
                                // } else {
                                //     weight = last_move.rows_removed + nlast_move.rows_removed;
                                // }
